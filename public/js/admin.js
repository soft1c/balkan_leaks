...
function handleFeaturedPersonClick() {
  var checkbox = document.getElementById('featuredPerson');
  var popup = document.getElementById('featuredPersonPopup');
  if (checkbox.checked) {
    fetchFeaturedPersons(); // Dohvati trenutno izdvojene osobe
  } else {
    popup.style.display = 'none';
  }
}

function fetchFeaturedPersons() {
  fetch('/get_featured_persons')
    .then(response => response.json())
    .then(featuredPersons => {
      const popup = document.getElementById('featuredPersonPopup');
      const layoutRow = popup.querySelector('.layout-row');
      layoutRow.innerHTML = ''; // Očisti prethodne osobe

      featuredPersons.forEach(person => {
        const personDiv = document.createElement('div');
        personDiv.className = 'featured-person layout-box';
        personDiv.textContent = person.ime + ' ' + person.prezime;
        personDiv.setAttribute('data-id', person.id);
        
        // Dodajte stilove ako su potrebni
        personDiv.style.border = '1px solid #ccc';
        personDiv.style.padding = '10px';
        personDiv.style.margin = '5px';
        personDiv.style.cursor = 'pointer';

        // Dodajte click event handler direktno
        personDiv.addEventListener('click', function() {
          setFeaturedPerson(person.id);
        });

        layoutRow.appendChild(personDiv);
      });
...
// Dodajte ovu funkciju za delegaciju događaja
document.getElementById('featuredPersonPopup').addEventListener('click', function(e) {
  if (e.target.classList.contains('featured-person')) {
    const personId = e.target.getAttribute('data-id');
    setFeaturedPerson(personId);
  }
});



function setFeaturedPerson(personId) {
  console.log('radi');
